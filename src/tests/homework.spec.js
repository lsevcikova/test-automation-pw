...
test("homework", async ({ page }) => {
  ...
  // Input with email
  await page
    .locator("body .card input#email")
    .fill("lucie.sevcikova108@gmail.com");
  // Input with password
  await page.locator("body .card input#password").fill("myAwesomePassword108");
  // Input with password
  await page
    .locator("body .card input#password-confirm")
    .fill("myAwesomePassword108");
  // Submit button
...
test("lesson 3", async ({ page }) => {
  ...
  console.log(
    "Forgot password href:" + (await forgotPassword.getAttribute("href"))
  );
  ...
  console.log(
    "Text of the login button is" + (await loginButton.textContent())
  );
...
async function getInputName(page) {
  return page.getByLabel("Jméno a příjmení");
}

async function getInputEmail(page) {
  return page.getByLabel("Email");
}

async function getInputPassword(page) {
  return page.getByLabel("Heslo");
}

async function getInputConfirmPassword(page) {
...
async function getErrorMessage(page) {
  return page.locator(".invalid-feedback").locator("strong");
}
...
async function register(page, name, email, password, confirmPassword) {
  const inputName = await getInputName(page);
  await inputName.fill(name);
  const inputEmail = await getInputEmail(page);
  await inputEmail.fill(email);
  const inputPassword = await getInputPassword(page);
  await inputPassword.fill(password);
  const inputConfirmPassword = await getInputConfirmPassword(page);
  await inputConfirmPassword.fill(confirmPassword);
...
test.describe("Registration page", { tag: "@smoke" }, () => {
  ...
  test(
    ...
    async ({ page }) => {
      ...
      await expect(errorMessage).toBeVisible(page);
      await expect(errorMessage).toHaveText(
